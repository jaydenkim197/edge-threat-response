...
| P0-05 | P0 | baseline detector 구조·모델·입력 규격 | output은 person/knife로 고정. COCO single-model은 sanity baseline, composite person+knife는 primary implementation proposal, unified 2-class는 person annotation 완전성 확보 시에만 후보 | composite adapter와 legacy weights의 PC CPU smoke는 통과; W4 사람 검수, Baseline v1 CUDA 학습, license, Orin latency·고정 입력/threshold 필요 | 실제 detector 검증 전 | contract/adapter `IMPLEMENTED`, topology `PROPOSAL` |
...
| P0-10 | P0 | 외부 dataset recipe | target domain은 약 3 m 고정형 indoor elevated CCTV로 `DECISION`. SOHAS=첫 public real-data 후보(README/License 충돌 해결 필요), DaSCI=중복 확인 뒤 보강 후보, ACF=접근성 확인 뒤 외부 image 평가 후보, Mock Attack=sequence/event 정답 확인 뒤 외부 평가 후보, Simuletic=별도 synthetic ablation 한정. 실제 채택·recipe는 역할별 검수 후 승인 | 출처·권리, knife 라벨, CCTV domain, negatives, exact/perceptual duplicate, camera/session leakage. Event 지표에는 연속 영상·start/end 정답 추가 | D2 import 전 | `PROPOSAL` |
