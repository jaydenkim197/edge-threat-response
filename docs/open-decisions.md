...
| P0-10 | P0 | 외부 dataset recipe | 먼저 legacy 128장 사람 검수로 L0 역할을 정한다. SOHAS=첫 public real-data 후보(README/License 충돌 해결 필요), DaSCI=중복 확인 뒤 보강 후보, ACF=접근성 확인 뒤 외부 image 평가 후보, Mock Attack=sequence/event 정답 확인 뒤 외부 평가 후보, Simuletic=별도 synthetic ablation 한정 | 출처·권리, knife 라벨, CCTV domain, negatives, exact/perceptual duplicate, camera/session leakage. Event 지표에는 연속 영상·start/end 정답 추가 | D2 import 전 | `PROPOSAL` |
