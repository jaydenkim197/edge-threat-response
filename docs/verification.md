...
| Runtime 언어 결정 | 현재 source/test/CLI/config inventory, Python·Hybrid C++·Core-only C++ 비용·위험·성능 경로 비교 | Git `3b5337c`, Windows PC | Python-first와 benchmark-triggered Hybrid C++ gate 결정; 시간·workload 값은 실측 아닌 추정치 | `docs/runtime-language-decision.md` | 경로 `DECISION`; Orin 성능·C++ 효과 `PLANNED` |
